    ...
    bindDynamicEvents: function() {
        ...
        this.items.on("mouseover", function() {
            ...
            if (b) {
                ...
                var h = e.attr("data-item-value")
                  , g = PrimeFaces.escapeHTML(h.replace(/\"/g, "'"));
                if (a.cfg.multiple) {
                    var c = '<li data-token-value="' + g + '" class="ui-autocomplete-token ui-state-active ui-corner-all ui-helper-hidden">';
                    ...
                    a.input.val(e.attr("data-item-label"));
                ...
                a.invokeItemSelectBehavior(f, h)
            }
            a.hide()
    ...
    setupForceSelection: function() {
        ...
        this.input.blur(function() {
            ...
            if (a.cfg.autoSelection && b && a.checkMatchedItem && a.items && !a.isTabPressed && !a.itemSelectedWithEnter) {
                var c = a.items.filter('[data-item-label="' + d + '"]');
                if (c.length) {
                    c.click()
                }
            }
...
PrimeFaces.widget.DataTable = PrimeFaces.widget.DeferredWidget.extend({
    ...
    saveCell: function(j) {
        var e = j.find("div.ui-cell-editor-input :input:enabled")
          , c = false
          , f = this;
        if (j.data("multi-edit")) {
            var h = j.data("old-value");
            for (var d = 0; d < e.length; d++) {
                var g = e.eq(d)
                  , a = g.val();
                if (g.is(":checkbox")) {
                    var b = a + "_" + g.is(":checked");
                    if (b != h[d]) {
                        c = true;
                        break
                    }
            ...
            c = (e.eq(0).val() != j.data("old-value"))
        }
        if (c) {
            f.doCellEditRequest(j)
        } else {
            f.viewMode(j)
        }
        if (this.cfg.saveOnCellBlur) {
    ...
    makeRowsDraggable: function() {
        ...
        this.tbody.sortable({
            ...
            helper: function(h, j) {
                var e = j.children()
                  , g = $('<div class="ui-datatable ui-widget"><table><tbody></tbody></table></div>')
                  , d = j.clone()
                  , c = d.children();
...
PrimeFaces.widget.SelectOneMenu = PrimeFaces.widget.DeferredWidget.extend({
    ...
    initContents: function() {
        ...
        this.input.data(PrimeFaces.CLIENT_ID_DATA, this.id);
        for (var c = 0; c < this.items.size(); c++) {
          ...
          , a = {
            ...
            onsuccess: function(e, c, d) {
                PrimeFaces.ajax.Response.handle(e, c, d, {
                    widget: b,
                    handle: function(h) {
                        var g = h.indexOf("</select>") + 9
                          , f = h.substring(0, g);
                        b.input.replaceWith(f);
                        b.itemsWrapper.append(h.substring(g, h.length))
                    }
                });
                return true
            },
...
PrimeFaces.widget.SelectCheckboxMenu = PrimeFaces.widget.BaseWidget.extend({
    ...
    createMultipleItem: function(i) {
        ...
        var c = i.prevAll("li.ui-selectcheckboxmenu-item-group")
          ...
          , h = g.next().html().trim()
          , d = h.length
          , f = d > 0 && h !== "&nbsp;" ? (a ? g.next().text() : g.next().html()) : g.val()
          , b = '<li class="ui-selectcheckboxmenu-token ui-state-active ui-corner-all" data-item-value="' + g.val() + '">';
...
PrimeFaces.widget.DefaultCommand = PrimeFaces.widget.BaseWidget.extend({
    init: function(a) {
        ...
        this.jqTarget.closest("form").off("keydown." + this.id).on("keydown." + this.id, function(d) {
            var c = $.ui.keyCode;
            if (d.which == c.ENTER || d.which == 108) {
...
PrimeFaces.widget.SplitButton = PrimeFaces.widget.BaseWidget.extend({
    ...
    bindEvents: function() {
        ...
        $(document.body).off(b).on(b, function(d) {
            if (c.menu.is(":hidden")) {
                return
            }
            var f = c.menu.offset();
            if (d.pageX < f.left || d.pageX > f.left + c.menu.width() || d.pageY < f.top || d.pageY > f.top + c.menu.height()) {
...
PrimeFaces.widget.OverlayPanel = PrimeFaces.widget.BaseWidget.extend({
    ...
    bindTargetEvents: function() {
        ...
        if (this.cfg.showEvent === this.cfg.hideEvent) {
            ...
            this.target.off(b).on(b, function(f) {
                d.toggle()
            })
        } else {
...
PrimeFaces.widget.Spinner = PrimeFaces.widget.BaseWidget.extend({
    ...
    bindEvents: function() {
        ...
        this.input.on("keydown.spinner", function(c) {
            ...
            a.updateValue();
            var c = $.ui.keyCode;
            var b = (PrimeFaces.env.isIE(11) || PrimeFaces.env.isLtIE(11)) && (d.which === c.ENTER || d.which === c.NUMPAD_ENTER);
...
PrimeFaces.widget.Tooltip = PrimeFaces.widget.BaseWidget.extend({
    ...
    bindGlobal: function() {
        ...
        var b = this;
        $(document).off(this.cfg.showEvent + " " + this.cfg.hideEvent, this.cfg.globalSelector).on(this.cfg.showEvent, this.cfg.globalSelector, function(f) {
            var c = $(this);
            if (c.prop("disabled")) {
                return
            }
            ...
            var h = c.attr("title");
            if (h) {
                c.data("tooltip", h).removeAttr("title")
            }
            var d = b.jq.children(".ui-tooltip-arrow");
            if (c.hasClass("ui-state-error")) {
                b.jq.children(".ui-tooltip-text").addClass("ui-state-error");
                d.addClass("ui-state-error")
            } else {
                d.removeClass("ui-state-error")
            }
            var g = c.data("tooltip");
            if (g) {
                if (b.cfg.escape) {
                    b.jq.children(".ui-tooltip-text").text(g)
                } else {
                    b.jq.children(".ui-tooltip-text").html(g)
                }
                b.globalTitle = g;
                b.target = c;
                b.show()
            ...
            if (b.globalTitle) {
                ...
                b.globalTitle = null;
                b.target = null;
                b.jq.children(".ui-tooltip-text").removeClass("ui-state-error")
            }
        });
    ...
    bindTarget: function() {
        ...
        if (this.cfg.delegate) {
            var b = "*[id='" + this.target.attr("id") + "']";
            $(document).off(this.cfg.showEvent + " " + this.cfg.hideEvent, b).on(this.cfg.showEvent, b, function(d) {
                if (c.cfg.trackMouse) {
                    c.mouseEvent = d
                }
                if ($.trim(c.jq.children(".ui-tooltip-text").html()) !== "") {
                    c.show()
                }
...
PrimeFaces.widget.VerticalTree = PrimeFaces.widget.BaseTree.extend({
    ...
    makeDraggable: function(b) {
        ...
        b.draggable({
            start: function(f, h) {
                if (h.helper) {
                    var e = $(f.target)
                      , g = $(e.data("dragsourceid")).data("widget")
                      , d = 20;
    ...
    makeDropPoints: function(b) {
        ...
        b.droppable({
            ...
            scope: a,
            drop: function(d, p) {
                var e = $(p.draggable.data("dragsourceid")).data("widget"), n = c, t = $(this), q = t.closest("li.ui-treenode-parent"), u = c.getRowKey(q), j = (e.id !== n.id), m = e.draggedSourceKeys, h = (c.cfg.dropCopyNode && c.shiftKey), s, k;
                if (m) {
                    s = e.findNodes(m)
                } else {
                    s = [p.draggable]
    ...
    makeDropNodes: function(b) {
        ...
        b.droppable({
            ...
            drop: function(r, l) {
                var m = $(l.draggable.data("dragsourceid")).data("widget"), k = c, v = $(this), d = v.closest("li.ui-treenode"), g = c.getRowKey(d), e = (m.id !== k.id), j = m.draggedSourceKeys, p = (c.cfg.dropCopyNode && c.shiftKey), t, n, o;
                if (j) {
                    t = m.findNodes(j)
                } else {
                    t = [l.draggable]
    ...
    fireDragDropEvent: function(c) {
        ...
        b.params = [{
            ...
            value: c.isDroppedNodeCopy
        }];
        if (this.cfg.controlled) {
            b.oncomplete = function(j, e, f) {
                if (f.access) {
                    for (var g = 0; g < d.droppedNodeParams.length; g++) {
                        var h = d.droppedNodeParams[g];
                        if (h.dropPoint) {
                            d.onDropPoint(h.ui, h.dragSource, h.dragNode, h.targetDragNode, h.dropPoint, h.dropNode, h.transfer)
                        } else {
                            d.onDropNode(h.ui, h.dragSource, h.dragNode, h.targetDragNode, h.droppable, h.dropNode, h.transfer)
                        }
                    }
                }
            }
...
PrimeFaces.widget.HorizontalTree = PrimeFaces.widget.BaseTree.extend({
    init: function(a) {
        this._super(a);
        if (PrimeFaces.isIE() && !this.cfg.disabled) {
            this.drawConnectors()
        }
    },
    bindEvents: function() {
...
PrimeFaces.widget.TreeTable = PrimeFaces.widget.DeferredWidget.extend({
    ...
    collapseNode: function(b) {
        ...
        b.attr("aria-expanded", false).find(".ui-treetable-toggler:first").addClass("ui-icon-triangle-1-e").removeClass("ui-icon-triangle-1-s");
        ...
        if (this.hasBehavior("collapse")) {
            var g = this
              ...
              , h = {
                ...
                onsuccess: function(l, i, k) {
                    PrimeFaces.ajax.Response.handle(l, i, k, {
                        widget: g,
                        handle: function(m) {}
                    });
                    return true
...
PrimeFaces.widget.Sidebar = PrimeFaces.widget.BaseWidget.extend({
    ...
    disableModality: function() {
        if (this.mask) {
            ...
            this.mask.remove();
            if (this.cfg.blockScroll) {
                a.removeClass("ui-overflow-hidden")
            }
            this.mask = null
        }
    },
